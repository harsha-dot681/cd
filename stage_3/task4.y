...
%token T_BEGIN T_END T_READ T_WRITE T_IF T_ENDIF T_then T_ELSE T_WHILE T_DO T_ENDWHILE LE GE NE EQ T_BREAK T_CONT
%nonassoc T_then
%nonassoc T_ELSE
%token<no>number ID
...
%left '*' '/'
%%
program: T_BEGIN Slist T_END {fptr=fopen("s3.xsm","w+");fprintf(fptr,"%d\n%d\n%d\n%d\n%d\n%d\n\%d\n%d\n",0,2056,0,0,0,0,0,0);fprintf(fptr,"MOV SP,4122\n");codegen1($2,fptr,-1,-1);;exit(0);}| T_BEGIN T_END {$$=NULL;}
;
Slist : Slist Stmt{$$=makeStmtNode(STATEMENT,$1,$2,"STATEMENTS");} |Stmt {$$=$1;}
;
Stmt : T_READ '('ID')'';' {$$=makeStmtNode($3->type,$3,NULL,"read");}| T_WRITE '('E')'';' {$$=makeStmtNode($3->type,$3,NULL,"write");} |ID '=' E';'{$$=makeExprNode($1->type,"=",$1,$3,"=");}|Ifstmt';'{$$=$1;}|Whilestmt';'{$$=$1;}|T_BREAK';'{$$=make_break_continue(BREAK,"break");}|T_CONT ';'{$$=make_break_continue(CONTINUE,"cont");}
;
